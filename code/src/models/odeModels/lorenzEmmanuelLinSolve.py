...
class LorenzEmmanuelLinSolve(object):
    def __init__(self):
    ...
    def __initNetwork(self):
        print("init network")
        self.inputDim = 20
        self.factorMatrix = np.zeros((self.inputDim,self.inputDim+self.inputDim*self.inputDim))


    def __evalF(self,uVals):
            nn = tf.multiply( self.nn1(uVals) + 1.0,self.nn2(uVals) + 1.0 )
            f = self.denseOut(nn)
    ...
    def __initComputationNetwork(self):
        with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
            ...
            self.nn1 = tf.layers.Dense(ww,kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=reg),dtype=self.dtypeVal)
            self.nn2 = tf.layers.Dense(ww,kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=reg),dtype=self.dtypeVal)
            self.nn3 = tf.layers.Dense(ww,kernel_regularizer=tf.contrib.layers.l2_regularizer(scale=reg),dtype=self.dtypeVal)


            self.denseOut = tf.layers.Dense(self.inputDim,dtype=self.dtypeVal)
            ### =========================================
    ...
    def genTargets(self,inputX,dh):
        self.evalUP1 = inputX+dh*(self.outputFP1)
    ...
    def __initTrainingVars(self):
        ...
        # self.outputTrueUP0 = tf.placeholder(self.dtypeVal, shape=[None, self.inputDim])
        ...
        self.lr = tf.get_variable("lr",initializer=tf.constant(0.01,self.dtypeVal),trainable=False,dtype=self.dtypeVal)
    ...
    def odeDerivative(self,t,y):
        ...
        evalKernel = self.predict(xs=y.reshape(1,-1))#*100.0
