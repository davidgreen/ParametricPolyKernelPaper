...
class LorenzFromANNDropFromDrv(object):
    def __init__(self):
    ...
    def __evalF(self,uVals):
            nn = tf.multiply( self.nn1(uVals),self.nn2(uVals) )
            f = self.denseOut(nn)
            return f
    ...
    def __initComputationNetwork(self):
        ...
        with tf.variable_scope("foo", reuse=tf.AUTO_REUSE):
            ...
            self.denseOut = tf.layers.Dense(3,dtype=self.dtypeVal)
            ### =========================================
    ...
    def __initTrainingVars(self):
        ...
        self.trueF = tf.placeholder(self.dtypeVal, shape=[None, self.inputDim])
        ...
        self.inputDH = tf.placeholder(self.dtypeVal, shape=[None, 3])
    ...
    def trainValues(self,numRepeats,lr,xs,xsp1,xsp2,xsp3,xsp4,xsp5,dh,trueF):

    	previousLoss = np.inf
        numTrainingVals = xs.shape[0]-1
        dhInput = np.zeros((xs.shape[0],3))
        dhInput += dh
