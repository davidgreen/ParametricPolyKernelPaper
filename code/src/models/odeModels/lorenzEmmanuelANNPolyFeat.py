...
class LorenzEmmanuelANNPolyFeat(object):
    def __init__(self,numDofs):
    ...
    def __evalF(self,uVals):
        ...
        polyFeatList = []
        for i in range(0,self.inputDim):
            for j in range(0,i+1):
                xi = tf.reshape(uVals[:,i],(-1,1))
                xj = tf.reshape(uVals[:,j],(-1,1))

                prod = xi*xj
                polyFeatList.append(prod)

        poly = tf.concat(polyFeatList,1)
        allFeatures = tf.concat([uVals,poly],1)
        f = self.nn1(allFeatures)
    ...
    def __initComputationNetwork(self):
        ...
        with tf.variable_scope("polyFeat", reuse=tf.AUTO_REUSE):
    ...
    def __initTrainingVars(self):
        ...
        self.lr = tf.get_variable("lrPolyFeat",initializer=tf.constant(0.01,self.dtypeVal),trainable=False,dtype=self.dtypeVal)
